...
- Create encrypted file using "ansible-vault" command. Use `123456` as password.
...
- Create a parameter named `ogulcan-vault_passwd` at AWS Parameter Store. Input `123456` as value.

- Modify the `vault_passwd.sh` file as below.

```sh
#!/bin/bash
aws --region=us-east-1 ssm get-parameters --names "ogulcan-vault_passwd" --query "Parameters[*].{Value:Value}" --output text
```
