...
- Part 1 - Build the Infrastructure (3 EC2 Instances with Ubuntu 24.04 AMI)
...
- Get to the AWS Console and spin-up 3 EC2 Instances with ```Ubuntu 24.04``` AMI.
...
- Copy the `phonebook` folder to the `ansible-lesson` folder in the control node from your GitHub repository.
...
- name: web server configuration
  ...
  tasks:
    ...
    - name: Install dependencies with --break-system-packages
      become: yes
      ansible.builtin.pip:
